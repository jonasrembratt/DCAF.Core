...
local DCAF_MobileGroupDefence_Defaults = {
    DefendersTemplatePrefix = "RED Mobile Defence"
}

DCAF.MobileDefence = {
    ClassName = "DCAF.MobileDefence",
    ---------------
    GroupName = nil,     -- group name of mobile group
    DefendersTemplatePrefix = DCAF_MobileGroupDefence_Defaults.DefendersTemplatePrefix,
...
function DCAF.MobileDefence:_removeDefenderSpawn(defender)
...
function DCAF.MobileDefence:Defend(threatUnit)
    self._isDefending = true
    self.Group:OptionAlarmStateRed()
    -- gwm.Group:RelocateGroundRandomInRadius(nil, 300)

-- Debug("nisse - DCAF.MobileDefence:Defend :: self.SpawnDefenders: " .. DumpPretty(self.SpawnDefenders))
...
-- Debug("nisse - DCAF.MobileGroupDefence:Defend_monitorDefendersHealth_hit :: lifeRemaining: " .. lifeRemaining)
    ...
    local delayDefence = math.random(10, 30) -- todo - link deployment speed to GROUP skill level
    -- Debug(self.ClassName .. " :: " .. self.GroupName .. " has detected " .. threatUnit.UnitName .. " :: deploys defence in " .. delayDefence .. " seconds")
    DCAF.delay(function()
...
function DCAF.MobileDefence:StartDetection()
-- Debug("nisse - DCAF.MobileGroupDefence:StartDetection...")
...
function DCAF.MobileDefence:ProcessDetectedHostiles(detectedUnits)
...
--- Creates and returns a #DCAF.MobileDefence - effectively making a group able to move and stop to defend itself
...
-- @param #string prefix - (optional; DCAF_MobileGroupDefence_Defaults.DefendersTemplatePrefix) Specifies groups to be spawned when defending
function DCAF.MobileDefence:New(group, maxDefenders, prefix)
    local md = DCAF.clone(DCAF.MobileDefence)
    if not isAssignedString(prefix) then
        prefix = DCAF_MobileGroupDefence_Defaults.DefendersTemplatePrefix
    end
    md.SpawnDefenders = getSpawnsWithPrefix(prefix, maxDefenders or 1)
    md.Group = getGroup(group)
    if not md.Group then
        return Warning("DCAF.MobileDefence:New :: group not found: " .. group) end
...
function DCAF.MobileDefence:Dispose(delay)
    self.Detection:__Stop(delay)
    self.Detection = nil
end

function DCAF.MobileDefence:GetEffectiveRange(threatUnits)
    -- todo DCAF.MobileDefence:GetEffectiveRange - use actual threat vs. effective range of defenders
...
function DCAF.MobileDefence:SetDefaultPrefix(prefix)
    if not isAssignedString(prefix) then
        error("DCAF.MobileGroupDefence:SetDefaultDefendersPrefix :: `prefix` must be assigned string, but was: " .. DumpPretty(prefix)) end

    DCAF_MobileGroupDefence_Defaults.DefendersTemplatePrefix = prefix
end